// Fig 10.7: Date.cpp
// Date class member-and friend-fucntion definitions.
#include <iostream>
#include <string>
#include "Date.h"
using namespace std;
...
void Date::setDate(int mm, int dd, int yy)
{
    ...
    else throw invalid_argument("Year is out of range");
...
} // end overloaded function operator++

// overloaded postfix increment operator; note that 
// dummy integer parameter does not have a parameter name
Date Date::operator++(int) 
{
    Date temp = *this; // hold the current state of object
    helpIncrement();

    // return unincremented, saved, temporary oject
...
bool Date::endOfMonth (int testDay) const
{
    if (month == 2 && leapYear ( year ))
        return testDay == 29; // last day of Fed in leapyear
    else
        return testDay == days[ month ];
} // end function endOfMonth

// function to help increment date
